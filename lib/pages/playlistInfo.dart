...
import 'package:spaceship_academy/data/playlistProvider.dart'; // Add this line

class PlaylistInfo extends StatefulWidget {
  final String playId; // ประกาศตัวแปร playId

  const PlaylistInfo({super.key, required this.playId});
      ...
      appBar: AppBar(
        ...
        title: Consumer<PlaylistProvider>(
          builder: (context, playlistProvider, child) {
            // หา playlist ที่ตรงกับ playId แล้วแสดงชื่อ
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              bool shouldFetchNewData = await Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) =>
                            PlaylistEdit(playId: widget.playId)),
                  ) ??
                  false;

              if (shouldFetchNewData) {
                fetchCourses(); // ทำการ fetch ข้อมูลใหม่
              }
            },
          )
        ],
